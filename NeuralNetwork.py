...
class NeuralNetwork():
    ...
    def backward_pass(self, inputs, targets, learning_rate):
        ...
        for i in range(self.n_out):
            ...
            #output_deltas[i] = ...
            if np.isscalar(targets):
                output_deltas[i] = (outputs[i] - targets) * sigmoid_derivative(sigmoid(outputs[i]))
            else:
                output_deltas[i] = (outputs[i] - targets[i]) * sigmoid_derivative(sigmoid(outputs[i]))
            ########################################/
        ...
        for i in range(len(hidden_deltas)):
            ...
            for k in range(len(output_deltas)):
                try:
                    sum_ +=  output_deltas[k] * self.w_hidden[i][k] 
                except:
                    pass
            hidden_deltas[i] = sigmoid_derivative(sigmoid(self.y_hidden[i]))*(sum_)
            ########################################/
